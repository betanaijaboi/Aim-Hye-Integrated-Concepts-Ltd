...
async function main() {
  ...
  // packaging: "glass" | "can" | "pet"
  // productFamily: groups variants of the same drink (different sizes/packaging)
  const products = [
    // ── Champion Breweries ────────────────────────────────────────────────
    ...
    // ── Nigerian Breweries ────────────────────────────────────────────────
    // Star Lager: 60cl glass, 33cl glass, 33cl can
    ...
    // Heineken: 60cl glass, 33cl glass, 33cl can
    ...
    // Fayrouz: 33cl glass, 50cl PET — two flavours each
    ...
    { name: "Fayrouz (Strawberry)", sku: "NB-FAYRS-33",    breweryId: nb.id, category: "malt", size: "33cl", packSize: 24, packaging: "glass", productFamily: "fayrouz-strawberry", pricePerCrate: 2040, pricePerBottle: 85, depositPerCrate: 480 },
    { name: "Fayrouz (Strawberry)", sku: "NB-FAYRS-50-PET",breweryId: nb.id, category: "malt", size: "50cl", packSize: 12, packaging: "pet",   productFamily: "fayrouz-strawberry", pricePerCrate: 1680, pricePerBottle: 140, depositPerCrate: 0 },
    ...
    // ── Guinness Nigeria ──────────────────────────────────────────────────
    // Guinness FES: 60cl glass, 33cl can
    ...
    // Harp Lager: 60cl glass, 33cl glass
    ...
    // Malta Guinness: 33cl glass, 33cl can
  ...
  for (const p of products) {
    await prisma.product.upsert({
      where: { sku: p.sku },
      // Always update packaging + productFamily so migrations backfill correctly
      update: { packaging: p.packaging ?? "glass", productFamily: p.productFamily ?? null },
